...
// never be one: IsRandomBot() returns false on IsSelfBot(), and IsAltBot()
// excludes it too, so the module gives it neither `grind`/`new rpg` nor the
// periodic randomization that teaches spells, refreshes gear and empties the bag.
bool PlayerbotAI::ActsAsRandomBot()
{
    return sPlayerbotAIConfig.selfBotActsAsRandomBot && IsSelfBot(bot);
}
